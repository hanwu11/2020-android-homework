package com.example.myfirstappshop;
...
import com.example.myfirstappshop.bean.UserInfo;
import com.example.myfirstappshop.database.UserDBHelper;
import com.example.myfirstappshop.util.DateUtil;
import com.example.myfirstappshop.util.ViewUtil;

public class LoadActivity extends AppCompatActivity implements View.OnClickListener, View.OnFocusChangeListener {
    ...
    private RadioButton rb_verification; // 声明一个单选按钮对象
    private EditText et_phone; // 声明一个编辑框对象
    private TextView sp_password; // 声明一个文本视图对象
    private EditText et_password; // 声明一个编辑框对象
    private Button bt_forget; // 声明一个忘记密码按钮控件对象
    private Button bt_login; // 声明一个登录按钮控件对象
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rb_verification = findViewById(R.id.rb_verification);
        et_phone = findViewById(R.id.et_phone);
        sp_password = findViewById(R.id.sp_password);
    ...
    // 初始化用户类型的下拉框
    private String[] typeArray = {"个人用户", "公司用户","18990064 武政澎"};
    ...
    private void initTypeSpinner() {
        ...
        Spinner sp_type = findViewById(R.id.tv_name);
        // 设置下拉框的标题
    ...
    private class RadioListener implements RadioGroup.OnCheckedChangeListener {
        ...
        public void onCheckedChanged(RadioGroup group, int checkedId) {
            if (checkedId == R.id.rb_password) { // 选择了密码登录
                sp_password.setText("登录密码：");
                et_password.setHint("请输入密码");
                bt_forget.setText("忘记密码");
                sw_remember.setVisibility(View.VISIBLE);
            } else if (checkedId == R.id.rb_verification) { // 选择了验证码登录
                sp_password.setText("　验证码：");
                et_password.setHint("请输入验证码");
                bt_forget.setText("获取验证码");
    ...
    private class HideTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable s) {
            ...
            if ((mStr.length() == 11 && mMaxLength == 11) || (mStr.length() == 6 && mMaxLength == 6)
                ...
                ViewUtil.hideOneInputMethod(LoadActivity.this, mView);
            }
        }
    }
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.bt_forget) { // 点击了“忘记密码”按钮
            ...
            } else if (rb_verification.isChecked()) { // 选择了验证码方式校验，此时要生成六位随机数字验证码
                // 生成六位随机数字的验证码,结果用0填充
        ...
        } else if (v.getId() == R.id.bt_login) { // 点击了“登录”按钮
            if (phone.length() < 11) { // 手机号码不足11位
                Toast.makeText(this, "请输入正确的手机号", Toast.LENGTH_SHORT).show();
                return;
            }
            ...
            } else if (rb_verification.isChecked()) { // 验证码方式校验
